...
class StudentAttendanceRequestScreen extends StatefulWidget {
  ...
  @override
  State<StudentAttendanceRequestScreen> createState() => _StudentAttendanceRequestScreenState();
}
...
class _StudentAttendanceRequestScreenState extends State<StudentAttendanceRequestScreen>
  ...
  void _startStatusPolling() {
    ...
    _statusTimer = Timer.periodic(const Duration(seconds: 15), (_) async {
      try {
        ...
        final resp = await ApiService.getSelfAttendanceStatus(studentToken: studentToken, classQr: classQr);
        final enabled = (resp['enabled'] == true);
        final expires = resp['expires_at'] is String ? DateTime.tryParse(resp['expires_at']) : null;
        if (!mounted) return;
        setState(() {
  ...
  // QR scanning/loading removed; QR values load automatically from saved profile
  ...
  Future<void> _requestAttendance() async {
    ...
    if (!_attendanceEnabled) {
      await _showInfoDialog(
        ...
        message: 'Your teacher hasn\'t enabled attendance yet. Please wait and try again.',
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      if (e.toString().contains('no_active_window')) {
        errorMessage = 'Self-attendance is not currently enabled by your teacher.';
        if (mounted) {
          await _showInfoDialog(
            title: 'Recording Not Enabled',
            message: 'Your teacher has not enabled student self-attendance for this class yet. Please wait until your teacher enables it and try again.',
          );
        }
      } else if (e.toString().contains('outside_geofence')) {
        errorMessage = 'You are not in the classroom location. Attendance cannot be recorded.';
                  ...
                  children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            const Expanded(
                              child: Text("Your teacher hasn't enabled attendance yet."),
                            ),
                          ],
                        ),
                      ),
                      const SizedBox(height: 12),
                    ] else if (_attendanceEnabled && (_lat == null || _lng == null)) ...[
                      Container(
                        padding: const EdgeInsets.all(12),
                        decoration: BoxDecoration(
                        ...
                        child: Row(
                          children: [
                            const Icon(Icons.gps_off, color: Colors.orange, size: 20),
                            const SizedBox(width: 8),
                            const Expanded(
                              child: Text('Location not available. Enable GPS and permissions to proceed.'),
                            ),
                            TextButton.icon(
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (_attendanceEnabled && _lat != null && _lng != null) ...[
                                  ...
                                  if (_attendanceEnabled && _windowExpiresAt != null) ...[
                                    const SizedBox(height: 4),
                                    Row(
                                      children: [
                                        const Icon(Icons.timer, size: 14, color: Color(0xFF10B981)),
                                        const SizedBox(width: 6),
                                        Text(
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(color: AppColors.shadowLight, blurRadius: 12, offset: const Offset(0, 6)),
                        ],
                      ),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.stretch,
                        children: [
                          const Text('Your QR codes',
                              style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                          const SizedBox(height: 16),
                          TextField(
                            controller: _studentQrController,
                            readOnly: true,
                            decoration: const InputDecoration(labelText: 'Student QR Code'),
                          ),
                          const SizedBox(height: 8),
                          const SizedBox(height: 12),
                          TextField(
                            ...
                            readOnly: true,
                            decoration: const InputDecoration(labelText: 'Class QR Code'),
                          ),
                          const SizedBox(height: 8),
                          const SizedBox(height: 16),
                          const SizedBox(height: 24),
                          ElevatedButton.icon(
                            onPressed: (_isLoading || _submitted) ? null : _requestAttendance,
                            icon: const Icon(Icons.how_to_reg),
                            label: Text(_submitted ? 'Recorded' : (_isLoading ? 'Submitting...' : 'Record Attendance')),
